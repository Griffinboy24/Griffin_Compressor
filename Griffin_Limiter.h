...
#define M_PI 3.14159265358979323846
    ...
    namespace FunctionsClasses {
        ...
        class DelaySmooth {
        private:
            const size_t bufferLen = 2ul << (8 * sizeof(uint16_t) - 1); // 65536
            size_t delay = 0;
        ...
        public:
            ...
            void Process(float** xVec, float** yVec, size_t vecLen) {
                // Move pointer lookups out of inner loop.
                float* xLeft = xVec[0];
                float* xRight = xVec[1];
                float* yLeft = yVec[0];
                float* yRight = yVec[1];
        ...
        //------------------------------------------------------------------------------
        // ExpSmootherCascade: Cascaded one-pole exponential smoothers (4 stages fixed)
    ...
    //------------------------------------------------------------------------------
    // SNEX Node Template - Stereo Limiter Node Implementation
    //------------------------------------------------------------------------------
    template <int NV>
    struct Griffin_Limiter : public data::base
    {
        ...
        // Use our DSP Limiter (stereo) in place of the example AudioEffect.
    ...
    public:
        ...
        void prepare(PrepareSpecs specs)
        {
            ...
            limiter.Reset();
            // Preallocate scratch buffers (use maxBlockSize if available, otherwise default to 512).
        ...
        inline void process(ProcessDataType& data)
        {
            ...
            // Use JUCE's vectorized copy routines.
